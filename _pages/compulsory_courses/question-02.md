...
A triple of efficient algorithms $(G, E, D)$ is a **public key encryption scheme** of length function $\ell: \mathbb{N} \to \mathbb{N}$ if:
...
- We select a public exponent $e$ co-prime to $\varphi(N) = (p - 1) \cdot (q - 1)$ and compute its multiplicative inversion $d$ in $\mathbb{Z}_{\varphi(N)}^\times$, i.e. $e \cdot d \equiv 1 \pmod{(p - 1) \cdot (q - 1)}$
...
The typical choices for $G$ are $\mathbb{Z}_p^\times$ for a large prime $p$ such that $p - 1$ is not smooth, or groups generated by elliptic curves.
...
## DSA
Digital signature algorithm is a digital signature scheme based on the discrete logarithm problem in $\mathbb{Z}_p^\times$.
...
Hash functions using the Merkle-Damgård Construction are vulnerable to **length extension attacks**.
...
together with the special point at infinity $\mc{O}$. In cryptography, we usually use $F = \mathbb{F}_p$ with prime $p \geq 5$.
